...
| Days Between Needs  | `/opt/venv/bin/python /app/manage.py days_between_needs`                      | `0 4 * * 0`           | Weekly on Sunday at 4:00 AM    |
| Task Worker         | `/opt/venv/bin/python /app/manage.py db_worker --batch --max-tasks 50 --queue-name *` | `*/5 * * * *`         | Every 5 minutes                |
