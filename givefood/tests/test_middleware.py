...
class TestGeoJSONPreloadMiddleware:
    ...
    def test_index_page_has_link_header(self, client):
        ...
        if response.status_code == 200:
            ...
            assert '/needs/geo.json' in response['Link'], "Link header should contain geojson URL"
            assert 'rel=preload' in response['Link'], "Link header should have rel=preload"
            assert 'as=fetch' in response['Link'], "Link header should have as=fetch"
            assert 'crossorigin=anonymous' in response['Link'], "Link header should have crossorigin=anonymous"
    ...
    def test_non_html_response_no_link_header(self, client):
        ...
        if response.status_code == 200:
            ...
            assert 'Link' not in response or response['Link'] == '', "Non-HTML response should not have Link header"
    ...
    def test_middleware_adds_correct_header_format(self):
        """Test that the middleware creates the correct Link header format."""
        ...
        assert '>; rel=preload; as=fetch; crossorigin=anonymous' in link_header, "Link header should have correct format"
